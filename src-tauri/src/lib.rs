...
mod bindings_codegen {
    ...
    /// generated file in sync and fails CI if the bindings are stale.
    #[test]
    fn export_typescript_bindings() {
        // The hand-written types this replaces modelled u64/usize as `number`,
        // so keep bigints as `number` to preserve the frontend contract. The
        // header suppresses `noUnusedLocals` on the generated helpers/globals so
        // the strict frontend `tsc` build stays green.
